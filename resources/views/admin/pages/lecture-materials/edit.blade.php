    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div class="my-auto">
                    <h5 class="page-title fs-21 mb-1">تعديل مادة تعليمية</h5>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.lecture-materials.update', $material->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select name="lecture_id" class="form-select" required>
                                            ...
                                            @foreach($lectures as $lecture)
                                                <option value="{{ $lecture->id }}" {{ old('lecture_id', $material->lecture_id) == $lecture->id ? 'selected' : '' }}>{{ $lecture->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('lecture_id')
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select name="type" class="form-select" required id="material_type">
                                            <option value="file" {{ old('type', $material->type) == 'file' ? 'selected' : '' }}>ملف</option>
                                            <option value="link" {{ old('type', $material->type) == 'link' ? 'selected' : '' }}>رابط</option>
                                            <option value="video" {{ old('type', $material->type) == 'video' ? 'selected' : '' }}>فيديو</option>
                                            <option value="audio" {{ old('type', $material->type) == 'audio' ? 'selected' : '' }}>صوت</option>
                                            <option value="image" {{ old('type', $material->type) == 'image' ? 'selected' : '' }}>صورة</option>
                                        </select>
                                        @error('type')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="col-md-6 mb-3" id="file_path_div">
                                        ...
                                        @if($material->file_path)
                                            <div class="mb-2">
                                                <small class="text-muted">الملف الحالي: {{ $material->file_name }}</small>
                                            </div>
                                        @endif
                                    ...
                                    <div class="col-md-6 mb-3" id="external_url_div" style="display: none;">
                                        ...
                                        <input type="url" name="external_url" class="form-control" value="{{ old('external_url', $material->external_url) }}" placeholder="https://example.com">
                                        @error('external_url')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="number" name="sort_order" class="form-control" min="0" value="{{ old('sort_order', $material->sort_order) }}">
                                        @error('sort_order')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $material->is_active) ? 'checked' : '' }}>
